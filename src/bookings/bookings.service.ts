...
export class BookingsService {
    ...
    async isBookingAvailable({
        ...
        if (!car) {
            throw new NotFoundException(
                `The car with id ${carId} was not found`,
            );
        }
        ...
        startDateWithOffset.setDate(
            startDateWithOffset.getDate() - bookingOffset,
        );
    ...
    async getReport({
        year,
        monthNumber,
    }: GetReportDto): Promise<ReportResult> {
        if (
            year < 0 ||
        ...
        carUsageMap.forEach((daysInRent, carId) => {
            ...
            carUsagePercentage[`${car.brand}(${car.licensePlate})`] =
                usagePercentage;
        });
